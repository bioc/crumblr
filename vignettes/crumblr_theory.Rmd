...
df_v <- df_res %>%
  pivot_longer(!tau&!targetCount&!countTotal, names_to="variable") %>%
  mutate(variable = factor(variable, c( "v_empirical", "v_theoretical_est"))) %>%
  arrange(tau, variable) 
...
```{r plot2, fig.height=6, fig.align = 'center'}  
df_sub <- df_v %>%
  filter(targetCount >= 2 & (countTotal - targetCount) >= 2)
